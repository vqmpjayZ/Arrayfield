...
if game["Run Service"]:IsStudio() then
	...
	http_request = nil
	syn = {protect_gui = false,request = false,}
	http = nil 
...
function Window:Prompt(PromptSettings)
    ...
    if not Prompt then
        warn("Prompt is missing in the GUI hierarchy!")
        return
    end
    ...
    PromptUI.Content.Text = PromptSettings.Content or "No Content Provided"
    PromptUI.Sub.Text = PromptSettings.SubTitle or ""
    PromptUI.Title.Text = PromptSettings.Title or "Title"

    for _, child in pairs(PromptUI.Buttons:GetChildren()) do
	...
	if PromptSettings.Actions then
        for name, info in pairs(PromptSettings.Actions) do
            ...
            Button.Name = name
            Button.TextLabel.Text = info.Name or "Unnamed Button"
            Button.Visible = true
            ...
            Button.Interact.MouseButton1Up:Connect(function()
                if not clicked then
                    ...
                    if not Success then
                        warn("Error in callback for button:", name, Response)
                    end
                    ClosePrompt(Prompt)
                end
            end)
...
function ClosePrompt(Prompt)
    if not Prompt then
        warn("Cannot close a non-existent Prompt.")
        return
    end

    TweenService:Create(Prompt, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {BackgroundTransparency = 1}):Play()
    local PromptUI = Prompt:FindFirstChild("Prompt")
    if PromptUI then
        TweenService:Create(PromptUI, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {BackgroundTransparency = 1}):Play()
        TweenService:Create(PromptUI.UIStroke, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {Transparency = 1}):Play()
        TweenService:Create(PromptUI.Title, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {TextTransparency = 1}):Play()
        TweenService:Create(PromptUI.Content, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {TextTransparency = 1}):Play()
        TweenService:Create(PromptUI.Sub, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {TextTransparency = 1}):Play()
    end
...
function Hide()
	...
	TweenService:Create(Main.BottomBar, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {Size = UDim2.new(0, 470, 0, 40), BackgroundTransparency = 1}):Play()
...
function Maximise()
	...
	TweenService:Create(Main.BottomBar, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {BackgroundTransparency = 0}):Play()
...
function Minimise()
	...
	TweenService:Create(Main.BottomBar, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {BackgroundTransparency = 1}):Play()
...
function RayfieldLibrary:CreateWindow(Settings)
		...
		function Tab:CreateKeybind(KeybindSettings)
			...
			UserInputService.InputBegan:Connect(function(input, processed)
				...
				if CheckingForKey then
					if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= RayfieldLibrary.MenuKeybind then
	...
	Main.BottomBar.Size = UDim2.new(0, 470, 0, 40)
	Main.BottomBar.BackgroundTransparency = 1
	
	if UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled or MobileEnabled == true then
		Elements.Size = UDim2.new(0, 475, 0, 229)
		Elements.Position = UDim2.new(0.5, 0, 0.58, 0)
		
		TabList.Size = UDim2.new(0, 490, 0, 36)
		TabList.Position = UDim2.new(0.5, 0, 0.5, 0)
		TabList.Parent = Main.BottomBar
	else
		Elements.Size = UDim2.new(0, 475, 0, 366)
		Elements.Position = UDim2.new(0.5, 0, 0.601, 0)
		
		TabList.Size = UDim2.new(0, 471, 0, 36)
		TabList.Position = UDim2.new(0.496, 0, 0.125, 0)
	end
...
UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == RayfieldLibrary.MenuKeybind and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
